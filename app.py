...
def fetch_stock_data(ticker, interval_key, is_usd=False, usd_rate=1.0):
    try:
        # Veri çekme (Hata toleransı için her hisse arasında kısa bir es)
        t = yf.Ticker(ticker)
        ...
        # Fiyat ve 13 EMA (Selçuk Gönençer Disiplini)
        ...
        # %12 ve altı ciddi sıkışma (fırtına öncesi sessizlik) kabul edilir
...
with c1: 
    st.metric("Piyasa", "NÖTR-POZİTİF", "0.4%")
with c2: 
    st.markdown(f"""
        <div style="background:#1a1c24; border:1px solid #2d2f39; padding:7px 12px; border-radius:8px; height:68px;">
            ...
            <span style="font-size:1.1rem; font-weight:700;">{vix_val}</span> 
            <span style="font-size:0.7rem; color:{vix_color}; font-weight:bold;">{vix_text}</span>
        </div>
    """, unsafe_allow_html=True)
with c3: 
...
if is_usd:
    try:
        u_ticker = yf.Ticker("USDTRY=X")
        usd_rate = u_ticker.history(period="1d")['Close'].iloc[-1]
    except:
    ...
    with tab:
        ...
        with st.spinner(f'{sk_adi} Analiz Ediliyor...'):
            ...
            for h in hisseler:
                ...
                time.sleep(0.1) # Yahoo engelini aşmak için kısa bekleme
        ...
        if not df.empty:
            ...
            st.warning("Veri şu an çekilemiyor, lütfen birazdan tekrar deneyin.")
